...
def main(args):
    ...
    print(f' [teacher pipeline check] ')
    teacher_pipe = AnimateDiffPipeline.from_pretrained("emilianJR/epiCRealism",
                                                       motion_adapter=teacher_motion_adapter,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            c_skip_prev, c_out_prev = scalings_for_boundary_conditions(timesteps_prev,
                                                                       timestep_scaling=args.timestep_scaling_factor)
            ...
            with torch.no_grad():
                ...
                if args.do_aesthetic_loss:
                    aesthetic_loss, aesthetic_rewards = aesthetic_loss_fn(
            ...
            with torch.no_grad():
                with torch.autocast("cuda", dtype=weight_dtype):
                    target_noise_pred = student_unet(x_prev.float(), timesteps_prev, timestep_cond=None,
                                                     encoder_hidden_states=prompt_embeds.float(), ).sample
                if args.motion_control:
                    student_motion_controller.reset()
                    teacher_motion_controller.reset()
                # This Is Good Result #
                pred_x_0 = get_predicted_original_sample(target_noise_pred, timesteps_prev, x_prev,
                                                         noise_scheduler.config.prediction_type, alpha_schedule,
                                                         sigma_schedule, )
            ...
            if args.loss_type == "l2":
                ...
                loss_unet_cd = torch.mean(
                    torch.sqrt((model_pred_cd.float() - target_cd.float()) ** 2 + args.huber_c ** 2) - args.huber_c)
