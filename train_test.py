...
def main(args):
    ...
    student_pipe = AnimateDiffPipeline.from_pretrained(args.pretrained_model_path,
                                                       ...
                                                       torch_dtpe=weight_dtype)
    student_noise_scheduler = LCMScheduler.from_config(student_pipe.scheduler.config, beta_schedule="linear")
    student_pipe.scheduler = student_noise_scheduler
    ...
    teacher_unet.requires_grad_(True)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for trained_key, trained_value in trained_value.items():
            ...
            for name, para, in student_unet.named_parameters():
                if 'motion' not in name:
                    if name not in student_dict :
                        student_dict[name] = []

                        student_dict[name].append((para.detach().cpu()) * 10)
                    else :
                        before = student_dict[name][0]
                        present = para.detach().cpu()
                        student_dict[name] = []
                        student_dict[name].append(para.detach().cpu())
                        equal_check = torch.equal(before, present)
                        print(f'epoch {epoch} {name} is equal : {equal_check}')
            torch.save(save_state_dict, os.path.join(output_dir, f"checkpoints/checkpoint_epoch{epoch}.pt"))
        if epoch != 0:
            before_epoch = epoch - 1

            before_state_dict = torch.load(os.path.join(output_dir, f"checkpoints/checkpoint_epoch{before_epoch}.pt"),
                                           map_location="cpu")
            present_state_dict = torch.load(os.path.join(output_dir, f"checkpoints/checkpoint_epoch{epoch}.pt"),
                                            map_location="cpu")
