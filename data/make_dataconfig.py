...
csv_dir = '/share0/dreamyou070/dreamyou070/MyData/video/webvid-10M/webvid-10M-csv/0_100.csv'
elems = []
elem = ['videoid','page_dir','name']
elems.append(elem)

def main() :
    base_folder_dir = '/share0/dreamyou070/dreamyou070/OneStepVideo/experiment_20240710/general_prompt_num_frames_16_0709'
    prompt_folders = os.listdir(base_folder_dir)
    for prompt_folder in prompt_folders :
        #print(f'prompt_folder : {prompt_folder}')
        p = prompt_folder.split('_')[-1]
        ...
        original_video_base_dir_trg = os.path.join(f'{original_video_base_dir}/prompt_{p}_seed_0.gif')
        ...
        new_video_dir = os.path.join(new_video_base, f'prompt_{p}_seed_0.gif')
        # copy
        shutil.copy(original_video_base_dir_trg, new_video_dir)
        videoid = f'prompt_{p}_seed_0'
        page_dir = f'{videoid}.gif'
        elem = [videoid,page_dir,name]
        #print(f'elem = {elem}')
        elems.append(elem)
    # write csv
