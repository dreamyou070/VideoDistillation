...
class DistillWebVid10M(Dataset):
    ...
    def get_batch(self, idx):
        ...
        video_dir = os.path.join(self.video_folder, f"{videoid}.gif")
        # /share0/dreamyou070/dreamyou070/MyData/video/webvid-10M/webvid-10M-partial-video/prompt_69_seed_0.gif
        # cannot read gif ?
        video_reader = Image.open(video_dir)


        video_length = video_reader.n_frames
        ...
        print(f'clip_length = {clip_length} | start_idx = {start_idx} | batch_index = {batch_index}')

        video_attrs = video_reader.__dict__
        print(f'video_attrs = {video_attrs}')
        """
        target_frames = []
        for idx in batch_index:
            target_frame = video_reader.seek(idx)
            print(f'type of target_frame = {type(target_frame)}')
            target_frame_np = np.array(target_frame)
            print(f'type of target_frame_np = {type(target_frame_np)} | shape = {target_frame_np.shape}')
            target_frames.append(target_frame_np)
        pixel_values = np.array(target_frames) # (16, 256, 256, 3)
        #pixel_values = torch.from_numpy(np.array(target_frames)).permute(0, 3, 1, 2).contiguous()
